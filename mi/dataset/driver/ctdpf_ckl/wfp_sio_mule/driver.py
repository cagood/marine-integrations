...
from mi.dataset.dataset_driver import MultipleHarvesterDataSetDriver, SingleFileDataSetDriver,\
    DataSetDriverConfigKeys, DriverStateKey, HarvesterType
...
#
from mi.dataset.parser.ctdpf_ckl_wfp import \
    CtdpfCklWfpParser, \
    CtdpfCklWfpDataParticle, \
    CtdpfCklWfpMetadataParticle
...
class CtdpfCklWfpDataSetDriver(SioMuleDataSetDriver):
    ...
    def _ctdpf_ckl_wfp_sio_mule_file_changed_callback(self, new_state):
        """
        Callback used by the ctdpf_ckl_wfp_sio_mule single file harvester called when a file change is detected.
        Store the new state in a queue.
        @param new_state: state of the changed file
        """
        self._file_changed_callback(new_state, DataTypeKey.CTDPF_CKL_WFP_SIO_MULE)

    def _save_ctdpf_ckl_wfp_sio_mule_parser_state(self, state):
        """
